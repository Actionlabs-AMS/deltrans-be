...
class CashAdvanceResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'driver_name' => $this->when(isset($this->driver_id) && $this->relationLoaded('driver'), function () {
                return $this->driver ? trim($this->driver->first_name . ' ' . $this->driver->last_name) : null;
            }),
            'helper_id' => $this->helper_id ?? null,
            'helper_name' => $this->when(isset($this->helper_id) && $this->relationLoaded('helper'), function () {
